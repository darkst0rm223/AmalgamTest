...
{
	return KillstreakMap[H::Entities.GetWeapon()->m_iItemDefinitionIndex()];
}
...
void CKillstreaker::ApplyKillstreak()
{
	if (Vars::Misc::KillstreakWeapon.Value)
	{
		if (const auto& pLocal = H::Entities.GetLocal())
		{
			...
			CBaseEntity* pWeapon = pLocal->m_hActiveWeapon();
			auto streaksP = reinterpret_cast<NetworkVar_m_nStreaks*>(pWeapon->m_nStreaks());
			streaksP->Set(0, GetCurrentStreak());
			streaksP->Set(1, GetCurrentStreak());
...
void CKillstreaker::PlayerDeath(IGameEvent* pEvent)
{
	...
	Killstreak++;
	KillstreakMap[H::Entities.GetWeapon()->m_iItemDefinitionIndex()]++;
